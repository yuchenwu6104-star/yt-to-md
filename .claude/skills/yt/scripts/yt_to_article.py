...
- **嚴禁「劇透式」串接（引述內容不准先講一遍）**：串接句只能載引述以外的資訊（背景、對照數字、這段話在回應什麼問題）。不要把下一句引述的論點、數字、比喻先摘要或改述一遍再貼引述，那等於同一句話連講兩次。沒有引述外的資訊可補時，一律退回最簡形式「X 說：」，不要為了「有資訊量」硬寫
  - ❌（劇透）`他認為資料中心的瓶頸在電力。他說：「資料中心能容納多少晶片，取決於可用的電力。」`
  - ✅（補引述外的資訊）`主持人問到 GPU 供給是否還是最大限制。他說：「資料中心能容納多少晶片，取決於可用的電力。」`
  - ✅（沒有可補的就最簡）`他說：「資料中心能容納多少晶片，取決於可用的電力。」`
  - 判準：把引述遮起來只看串接句，如果讀者已經知道講者要說什麼，就是劇透，刪掉重寫
