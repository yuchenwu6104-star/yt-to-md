...
> **這一頁描述的是舊版（模型直接寫文章）。** 2026-08-06 起排程實跑的是地圖版
> `scripts/ytmap/yt_triage.py`（產分流稿＋地圖＋帶行號逐字稿，模型不寫文章），
> `/humanizer-zh <URL>` 也走那一支。舊版留著是退路：模型一次要做完翻譯／結構／
> 歸屬／專名／文筆五件事，實測同一集跑兩次，三整段虛構全部出現在寫文章那一步。
> 要文章請走 `/humanizer-zh <URL>`；只有明確要退回舊版時才用下面的指令。
